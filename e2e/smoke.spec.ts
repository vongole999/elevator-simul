...
test("시작을 누르면 그 건물의 1층 로비에서 시작한다", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "1층 로비" })).toBeVisible();
...
test("호출 버튼을 누르면 문이 열리고, 잠시 뒤 안쪽 시점으로 바뀐다", async ({ page }) => {
  ...
  // 승차 대기(4초 안팎)가 끝나면 화면이 캐빈 안쪽 시점으로 바뀐다(헤더가 "엘리베이터"로 바뀐다).
  // 타임아웃을 넉넉히 두는 이유는 기다림 자체가 아니라, 여러 e2e 테스트가
  // 동시에 브라우저를 띄울 때 생기는 리소스 경합을 흡수하기 위해서다.
  await expect(page.getByRole("heading", { name: "엘리베이터" })).toBeVisible({ timeout: 15000 });
